...
```
```{r}
final <- final %>% arrange(City, Occupation)
...
no_repeats <- no_repeats %>% filter(!(City %in% NA))
View(no_repeats)
...
#predicting average salary based on the other metrics

salary_lm <- lm(salary ~ apt_in_center + apt_out_center + basics + internet + inexpensive_meal, data = rows_to_cols)
summary(salary_lm)

fit_rpart <- rpart(salary ~ apt_in_center + apt_out_center + basics + internet + inexpensive_meal, data = rows_to_cols) 
rpart.plot(fit_rpart, digits = 4)

fit <- tree(salary ~ apt_in_center + apt_out_center + basics + internet + inexpensive_meal, data = rows_to_cols)
plot(fit, type = "uniform")
text(fit, cex = 1)

#the rpart plot is prettier, but the second tree gives more info. 
